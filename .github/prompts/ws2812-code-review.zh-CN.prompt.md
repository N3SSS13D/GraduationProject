...
1. 在下结论前，先总结当前实现切片和实际变更路径。
2. 先识别具体风险、回归点和缺失验证，再考虑修复建议。
3. 每条 finding 都优先给出最小且有证据支撑的解释。
4. 每发现一类主要问题后，回看受影响执行链路，避免审查漂成大范围猜测。
5. 如果代码更新了预期行为，但文档、Prompt 或 Skill 没有同步，明确指出这类漂移。
