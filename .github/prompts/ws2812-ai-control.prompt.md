...
14. When the task is a compact `16x16` animation sequence, prefer a dedicated MCP tool that emits `matrix_frame_sequence_v1` with LED-ready `bitmap_rows_hex` frames. For `10 fps`, prefer `10` frames at `100 ms` intervals.
