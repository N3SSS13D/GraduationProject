...
- HC-05 默认配置流：先在 `38400` 下发送 `AT` 探测；若连续 `3` 次无应答，则直接切本地串口到 `115200` 并跳过后续设置；若探测成功，再按“设置一条、查询一条”完成全部 AT 指令，AI端 使用固定地址 `98:D3:02:96:A2:B1` 对应的 `AT+BIND` 绑定 LED端，最后两步固定为 `AT+RESET` 和本地切到 `115200` 数据模式，名称统一为 `XiaoZhi -> WS2812`。
