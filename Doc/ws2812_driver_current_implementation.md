...
- 扫描总行数 16。
- 两套扫描方案：
  - `classic`（way2双行并行DMA）：每个扫描周期同时点亮两行（0/1、2/3 ...）。
    - CH1/P10 通过DMA输出前一行缓冲。
    - CH2/P12 通过DMA输出后一行缓冲。
    - 每周期后切换到下两行。
  - `quad`（四行窗口）：`row1/row3` 显示纯色，`row0/row2` 为清除行。
...
- P32 (INT0): 在 `classic` / `quad` 扫描方案间切换。
- P33 (INT1): 打开/关闭时序日志打印。
