...
static volatile uint16_t g_ws2812RefreshCount = 0U;
static volatile uint16_t g_ws2812DmaTriggerCount = 0U;
static volatile uint16_t g_ws2812DmaDoneCount = 0U;
static volatile uint16_t g_ws2812DmaTimeoutCount = 0U;
...
static void WS2812DRV_InitDualRowDmaBuffer(void)
{
	...
	/* PWMAT DMA expects the interleaved CH1/CH2 stream to start on an even address. */
	g_ws2812LastTxAddr = (uint16_t)g_ws2812DualRowPwmBuf;
}
...
static uint16_t WS2812DRV_FillDualRowPwmBuffer(uint8_t xdata *dualBuf, uint8_t bufIdx, uint8_t rowA, uint8_t rowB)
{
	uint16_t idx;
	uint16_t outIdx;
    uint16_t activePwmNum;
	...
	if ((dualBuf == 0) || (rowA >= WS2812DRV_ROW_NUM) || (rowB >= WS2812DRV_ROW_NUM))
	{
		return 0;
	}
...
{
	return WS2812DRV_FillDualRowPwmBuffer(g_ws2812DualRowPwmBuf, bufIdx, rowA, rowB);
}
...
void WS2812DRV_Init(void)
{
	...
	g_ws2812RefreshCount = 0U;
	g_ws2812DmaTriggerCount = 0U;
	g_ws2812DmaDoneCount = 0U;
	g_ws2812DmaTimeoutCount = 0U;
	g_ws2812DmaOddAddrCount = 0U;
}
...
void WS2812DRV_ClearImage(void)
{
	...
	{
		/* Keep a software fallback so frame rebuild still succeeds when M2M zero-fill is unavailable. */
		for (row = 0; row < WS2812DRV_ROW_NUM; row++)
		{
			for (col = 0; col < WS2812DRV_COL_NUM_MAX; col++)
			{
				for (colorIdx = 0; colorIdx < WS2812DRV_PIXEL_CHANNELS; colorIdx++)
				{
					g_ws2812ImageBuf[WS2812DRV_BUF_BACK][row][col][colorIdx] = 0U;
				}
			}
		}
	}
...
void WS2812DRV_TriggerDualRowDma(uint8_t xdata *txBuf, uint16_t num)
{
	...
	addr = (uint16_t)txBuf;
	g_ws2812LastTxAddr = addr;
	if ((addr & 0x0001U) != 0U)
	{
		g_ws2812DmaOddAddrCount++;
		WS2812DRV_BlankOutputs();
		return;
	}
