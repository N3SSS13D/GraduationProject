...
#define KEYCTRL_COMBO_PRESS_TICKS       200U
...
static uint16_t g_comboPressTicks = 0U;
static uint8_t g_comboHandled = 0U;
...
void KeyCtrl_Task10ms(void)
{
    ...
    /* P32: short press switches pattern, 0.8s long press toggles text/clock. */
    ...
    /* P32 + P33 simultaneous 2s press toggles USB debug mode. */
    bothPressed = (uint8_t)((g_key0State.pressed != 0U) && (g_key1State.pressed != 0U)
                            && (P32 == 0) && (P33 == 0));

    if (bothPressed != 0U)
    {
        if (g_comboPressTicks < 0xFFFFU)
        {
            g_comboPressTicks++;
        }

        if ((g_comboPressTicks >= KEYCTRL_COMBO_PRESS_TICKS) && (g_comboHandled == 0U))
        {
            GpLedMatrixUsbDebug_Toggle();
            g_comboHandled = 1U;
        }
    }
    else
