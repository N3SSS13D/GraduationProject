...
constexpr size_t kBtMaxPacketBytes = GP_MATRIX_PACKET_MAX_SIZE;
...
public:
    ...
    bool ReadPacket(uint8_t* data, size_t length, size_t* packet_length, uint32_t timeout_ms) override {
        GpMatrixRxPacket packet = {};

        if ((data == nullptr) || (length == 0U) || (packet_length == nullptr)) {
            return false;
        }
...
private:
    ...
    bool TryExtractPacket(GpMatrixRxPacket* packet) {
        const GpMatrixPacketHeader* header;
        ...
        while (true) {
            ...
            if (GpMatrixComputeHeaderCrc8(rx_buffer_.data(), GP_MATRIX_PACKET_HEADER_CRC_BYTES)
                != header->header_crc8) {
                ESP_LOGW(TAG, "HC-05 UART bad header crc");
                rx_buffer_.erase(rx_buffer_.begin());
                continue;
            }
            ...
            packet_crc = GpMatrixComputePacketCrc16(rx_buffer_.data(),
                                                    static_cast<uint16_t>(packet_length - GP_MATRIX_PACKET_TRAILER_SIZE));
            if (packet_crc != GP_MATRIX_READ_LE16(&rx_buffer_[packet_length - GP_MATRIX_PACKET_TRAILER_SIZE])) {
                ESP_LOGW(TAG, "HC-05 UART bad packet crc cmd=%s", CommandName(header->command));
