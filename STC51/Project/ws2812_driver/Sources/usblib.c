...
extern void Test_RequestLedCount(uint8_t ledCount);
...
void USBLIB_OUT_Callback(void)
{
    ...
    ledCount = 0;
    hasDigit = 0;
    for (idx = 0; idx < OutNumber; idx++)
    {
        if ((UsbOutBuffer[idx] >= '0') && (UsbOutBuffer[idx] <= '9'))
        {
            hasDigit = 1;
            ledCount = (uint16_t)(ledCount * 10 + (UsbOutBuffer[idx] - '0'));
        }
        else if (hasDigit != 0)
        {
            break;
    ...
    if (hasDigit != 0)
    {
        Test_RequestLedCount((uint8_t)ledCount);
    }
