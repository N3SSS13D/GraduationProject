...
typedef struct os_q {                       /* QUEUE CONTROL BLOCK                                     */
    struct os_q OsMemType  *OSQPtr;         /* Link to next queue control block in list of free blocks */
...
//    void            *OSTCBExtPtr;           /* Pointer to user definable data for TCB extension        */
    ...
    INT16U           OSTCBStkSize;          /* Size of task stack (in number of stack elements)        */
    INT16U           OSTCBOpt;              /* Task options as passed by OSTaskCreateExt()             */
    INT16U           OSTCBId;               /* Task ID (0..65535)                                      */
#endif

    struct os_tcb OsMemType  *OSTCBNext;    /* Pointer to next     TCB in the TCB list                 */
    struct os_tcb OsMemType  *OSTCBPrev;    /* Pointer to previous TCB in the TCB list                 */
...
//    OS_STK          *OSTCBStkBase;          /* Pointer to the beginning of the task stack              */     
    INT16U           OSTCBStkUsed;          /* Number of bytes used from the stack                     */
#endif
...
OS_EXT  INT32U  OsMemType       OSCtxSwCtr;               /* Counter of number of context switches           */

#if (OS_EVENT_EN) && (OS_MAX_EVENTS > 0u)
OS_EXT  OS_EVENT *  OsMemType   OSEventFreeList;          /* Pointer to list of free EVENT control blocks    */
OS_EXT  OS_EVENT OsMemType      OSEventTbl[OS_MAX_EVENTS];/* Table of EVENT control blocks                   */
#endif

#if (OS_FLAG_EN > 0u) && (OS_MAX_FLAGS > 0u)
OS_EXT  OS_FLAG_GRP   OsMemType OSFlagTbl[OS_MAX_FLAGS];  /* Table containing event flag groups              */
OS_EXT  OS_FLAG_GRP * OsMemType OSFlagFreeList;           /* Pointer to free list of event flag groups       */
#endif

#if OS_TASK_STAT_EN > 0u
OS_EXT  INT8U   OsMemType       OSCPUUsage;               /* Percentage of CPU used                          */
OS_EXT  INT32U  OsMemType       OSIdleCtrMax;             /* Max. value that idle ctr can take in 1 sec.     */
OS_EXT  INT32U  OsMemType       OSIdleCtrRun;             /* Val. reached by idle ctr at run time in 1 sec.  */
OS_EXT  BOOLEAN OsMemType       OSStatRdy;                /* Flag indicating that the statistic task is rdy  */
...
OS_EXT  OS_PRIO data            OSRdyGrp;                        /* Ready list group                         */
OS_EXT  OS_PRIO data            OSRdyTbl[OS_RDY_TBL_SIZE];       /* Table of tasks which are ready to run    */

OS_EXT  bit                     OSRunning;                       /* Flag indicating that kernel is running   */

OS_EXT  INT8U OsMemType         OSTaskCtr;                       /* Number of tasks created                  */

OS_EXT  volatile  INT32U OsMemType OSIdleCtr;                              /* Idle counter                   */

#ifdef OS_SAFETY_CRITICAL_IEC61508
OS_EXT  BOOLEAN OsMemType       OSSafetyCriticalStartFlag;
#endif

OS_EXT  OS_STK                  OSTaskIdleStk[OS_TASK_IDLE_STK_SIZE];      /* Idle task stack                */


OS_EXT  OS_TCB  * data          OSTCBCur;                        /* Pointer to currently running TCB         */
OS_EXT  OS_TCB  * OsMemType     OSTCBFreeList;                   /* Pointer to list of free TCBs             */
OS_EXT  OS_TCB  * data          OSTCBHighRdy;                    /* Pointer to highest priority TCB R-to-R   */
OS_EXT  OS_TCB  * data          OSTCBList;                       /* Pointer to doubly linked list of TCBs    */
OS_EXT  OS_TCB  * OsMemType     OSTCBPrioTbl[OS_LOWEST_PRIO + 1u];    /* Table of pointers to created TCBs   */
OS_EXT  OS_TCB    OsMemType     OSTCBTbl[OS_MAX_TASKS + OS_N_SYS_TASKS];   /* Table of TCBs                  */

#if OS_TICK_STEP_EN > 0u
OS_EXT  INT8U OsMemType         OSTickStepState;          /* Indicates the state of the tick step feature    */
#endif

#if (OS_MEM_EN > 0u) && (OS_MAX_MEM_PART > 0u)
OS_EXT  OS_MEM  * OsMemType     OSMemFreeList;            /* Pointer to free list of memory partitions       */
OS_EXT  OS_MEM    OsMemType     OSMemTbl[OS_MAX_MEM_PART];/* Storage for memory partition manager            */
#endif

#if (OS_Q_EN > 0u) && (OS_MAX_QS > 0u)
OS_EXT  OS_Q    * OsMemType     OSQFreeList;              /* Pointer to list of free QUEUE control blocks    */
OS_EXT  OS_Q      OsMemType     OSQTbl[OS_MAX_QS];        /* Table of QUEUE control blocks                   */
#endif

#if OS_TASK_REG_TBL_SIZE > 0u
OS_EXT  INT8U OsMemType         OSTaskRegNextAvailID;     /* Next available Task register ID                 */
#endif

#if OS_TIME_GET_SET_EN > 0u
OS_EXT  volatile  INT32U OsMemType OSTime;                /* Current value of system time (in ticks)         */
#endif

#if OS_TMR_EN > 0u
OS_EXT  INT16U OsMemType        OSTmrFree;                /* Number of free entries in the timer pool        */
OS_EXT  INT16U OsMemType        OSTmrUsed;                /* Number of timers used                           */
OS_EXT  INT32U OsMemType        OSTmrTime;                /* Current timer time                              */

OS_EXT  OS_EVENT * OsMemType    OSTmrSem;                 /* Sem. used to gain exclusive access to timers    */
OS_EXT  OS_EVENT * OsMemType    OSTmrSemSignal;           /* Sem. used to signal the update of timers        */

OS_EXT  OS_TMR   OsMemType      OSTmrTbl[OS_TMR_CFG_MAX]; /* Table containing pool of timers                 */
OS_EXT  OS_TMR * OsMemType      OSTmrFreeList;            /* Pointer to free list of timers                  */
OS_EXT  OS_STK                  OSTmrTaskStk[OS_TASK_TMR_STK_SIZE];

OS_EXT  OS_TMR_WHEEL OsMemType  OSTmrWheelTbl[OS_TMR_CFG_WHEEL_SIZE];
#endif
