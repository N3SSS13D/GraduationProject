...
## GraduationProject スナップショット補足

このリポジトリでは、`External/xiaozhi-esp32/` を卒業設計向けの参照スナップショットとして利用しています。目的は上流機能の置き換えではなく、`STC AI8051U + WS2812` 系統へ音声対話、MCP、I2C 拡張の実装参考を提供することです。

プロジェクト固有の拡張資産は `GP_Port/` にあり、主な内容は次のとおりです。

- `gp_led_matrix_protocol.h`：ESP32 と AI8051U の共有プロトコル
- `gp_led_matrix_esp32.h/.cc`：ESP32 側 LED マトリクスドライバ骨格
- `gp_led_matrix_ai8051u.h`：AI8051U 側インターフェース境界
- MCP デバッグツール、デバッグドット表示、段階別 prompt

次のマイルストーンは、小智 AI の音声結果をカスタム I2C プロトコル経由で AI8051U と WS2812 表示へ接続することです。
