...
- `lichuang-dev` 稳定版使用 `GPIO1/GPIO2` 作为矩阵链路复用 I2C 总线。
- 共享协议头统一使用地址 `0x31`。
- 新增 `gp_led_matrix_esp32.h/.cc` 作为 ESP32 侧矩阵驱动，并在目标板上通过 `GetLed()` 接入。
- AI8051U 侧已落地 `gp_led_matrix_ai8051u.c/.h`；稳定版为 I2C 从机实现，`BT_Version` 已改为 `UART2 + HC-05` 收包、协议解析和 WS2812 动作分发。

### 已实现内容

1. ESP32 侧状态驱动矩阵链路已经接入活跃板型构建系统。
2. 目标板已经声明矩阵 I2C 地址、默认亮度和实际连线引脚。
3. 设备侧可将语音结果与调试圆点状态映射为矩阵动作对象；稳定版通过 I2C 协议发送。`BT_Version` 已补充传输抽象层和条件编译的经典蓝牙 SPP 后端，但当前 `esp32s3` 目标仍以 I2C 作为可运行链路。
4. 当前链路遵循“仅显式图像更新时通信”，不会因待机/聆听等状态自动覆盖上一幅图像。
5. AI8051U 侧稳定版已显式切换 I2C 到 `P2.3/P2.4`，`BT_Version` 当前已改为 `UART2 P4.2/P4.3` 对接 `HC-05`。
...
9. `lichuang-dev` 当前已具备稳定版调试界面：主界面右下角 `DBG` 入口、固定标题栏 `Back / Debug Menu / S`、单页触摸控制区、圆点预览区、链路状态区和摘要信息区。
10. 设备侧截图已切换为本地 HTTP 上传链路；主机侧可通过 `/control/snapshot` 请求设备执行截图任务。

### 未完成内容

1. `BT_Version` 仍未完成 ESP32 侧蓝牙链路适配；当前只完成了 AI8051U 侧 UART2 传输改造。
2. 现阶段矩阵图案仍以代码生成和现有 `test_image.h` 索引为主，未建立更高层素材管理。
3. 当前滚动字幕仍依赖 AI8051U 侧内置字模序列，尚未支持自由文本下发。
4. 仍需补充更完整的实机联调记录与异常场景验证。
5. 心跳超时、断链自动恢复和更细化的链路重试策略仍待补充。
6. 已实测验证：当前 `ESP-IDF v5.4.3 + esp32s3` 目标无法把经典蓝牙 SPP 后端链接为可运行映像，因此当前板型仍无法完成“小智蓝牙 -> HC-05”无线闭环。
